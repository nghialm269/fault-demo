// Package errctx facilitates storing key-value data into
...
//		"post_id", postID,
//	)
func WithMeta(ctx context.Context, kv ...any) context.Context {
	if ctx == nil {
		return nil
	}
...
//	if err != nil {
//		return nil, errctx.Wrap(err, ctx, "role", "admin")
//	}
//
...
//	user, err := database.GetUser(ctx, userID)
//	if err != nil {
//		return nil, errctx.Wrap(errors.Wrap(err, "failed to get user data"),
//			ctx,
//			"role", "admin")
//	}
func Wrap(err error, ctx context.Context, kv ...any) error {
	if err == nil || ctx == nil {
		return err
	}
...
func createMeta(ctx context.Context, kv ...any) map[string]any {
	meta := make(map[string]any)

	if parent, ok := ctx.Value(contextKey{}).(map[string]any); ok {
...
func extractKV(kv ...any) (k string, v any, rest []any) {
	switch x := kv[0].(type) {
	case string:
		if len(kv) == 1 {
			return badKey, x, nil
		...
		return x, kv[1], kv[2:]
	default:
		return badKey, x, kv[1:]
	}
}

// With implements the Fault Wrapper interface.
func With(ctx context.Context, kv ...any) func(error) error {
...
// string to any map. This data can then be used in your logger of choice, or
...
//		})
//	}
func Unwrap(err error) map[string]any {
...
func GetMeta(ctx context.Context) map[string]any {
	...
	meta, ok := ctx.Value(contextKey{}).(map[string]any)
	if !ok {
		return nil
	}
